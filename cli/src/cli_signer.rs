...
impl CliSigner {
    pub fn new(
        ...
        remote_keypair: Option<RemoteKeypair>,
    ) -> anyhow::Result<Self> {
        if keypair.is_none() && remote_keypair.is_none() {
            return Err(anyhow!("No keypair or wallet manager provided"));
        }
    ...
    pub fn new_keypair(keypair: Keypair) -> Self {
        Self::new(Some(keypair), None).expect("CliSigner::new_keypair: keypair must be provided")
    }
    ...
    pub fn new_keypair_from_path(keypair_path: &str) -> anyhow::Result<Self> {
        match read_keypair_file(keypair_path) {
            Ok(keypair) => Self::new(Some(keypair), None),
            Err(e) => Err(anyhow!("{e}")),
        }
    }
    ...
    pub fn new_ledger() -> Self {
        ...
        Self::new(None, Some(remote_keypair))
            .expect("CliSigner::new_keypair: remote_keypair must be provided")
    }
}
